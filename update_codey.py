#!/usr/bin/env python3
# update_codey.py - Production Ready Edition
import requests
import json
import os
import sys
...
def analyze_repo_quality(repo_data):
    ...
    quality_factors = {
        'has_readme': True,  # Assume true since GitHub doesn't offer a clean API field (Fix #4)
    ...
    if quality_factors['is_fork']:
        score *= 0.5  # Forks are worth half the value (Fix #5)
...
def calculate_skill_decay(last_update_str, current_stats):
    ...
    try:
        ...
        time_inactive = datetime.now(timezone.utc) - last_update
        
        # Fix #3: Decay only occurs if the time difference is greater than 25 hours.
        if time_inactive < timedelta(hours=25): 
            return current_stats
        
        days_inactive = time_inactive.days + time_inactive.seconds / 86400.0 # Precise fractional days
        ...
        # Fix #6: Removed max(0.5, ...) to allow full decay
        decayed_stats['energy'] *= (0.9 ** days_inactive) 
        
        # Streak penalty for gaps
        if days_inactive > 2:
            decayed_stats['streak'] = max(0, decayed_stats['streak'] - int(days_inactive) + 1)
...
def get_all_data_for_user(owner):
    ...
    # Language analysis (Fix #8: Alle Repos analysieren)
...
def update_brutal_stats(codey, daily_activity, all_time_data, user_data):
    ...
    # Update stats with brutal modifiers (Fix #1 & #7)
    # 1. Gain from activity
    codey['hunger'] = min(100, codey['hunger'] + commit_xp * 0.8 + pr_xp * 1.2)
    codey['happiness'] = min(100, codey['happiness'] + pr_xp * 0.6)
    
    # 2. Loss from activity and passive decay
    energy_consumed = daily_activity['commits'] * 1.5 + daily_activity['prs'] * 3 + 5 # Passive loss of 5
    codey['energy'] = max(0, codey['energy'] - energy_consumed)
    codey['hunger'] = max(0, codey['hunger'] - 15)  # Passive hunger decay
    codey['happiness'] = max(0, codey['happiness'] - 8)  # Passive happiness decay
...
def get_seasonal_bonus():
    ...
    if month == 11: return {'emoji': '🍁', 'name': 'End-of-Year Push', 'multiplier': 1.25} # Fix #2
...
def generate_brutal_svg(codey, seasonal_bonus):
    ...
    # Fix #9: Use stored last_update instead of current time
    last_update_display = 'Never'
    if codey.get('last_update'):
        try:
            # Parse ISO string and reformat for display
            dt_object = datetime.fromisoformat(codey['last_update'].replace('Z', '+00:00'))
            last_update_display = dt_object.strftime('%Y-%m-%d %H:%M UTC')
        except:
            last_update_display = 'Invalid Time'
...
if __name__ == "__main__":
    ...
    print("\n✅ Codey is now production-ready! No more DevPlööts! ✅")
