...
def generate_brutal_svg(codey, seasonal_bonus):
    ...
    # 2. METRIK-BERECHNUNG (ASCII-BALKEN)
    ...
    if codey.get('achievements'):
        for i, ach in enumerate(codey['achievements'][-5:]):
            ...
            ach_xml += f'''
                <circle cx="{x}" cy="30" r="19" fill="#0c0018" stroke="{col}" stroke-width="1.5" filter="url(#glow)"/>
                <text x="{x}" y="37" text-anchor="middle" font-size="17">{ach.split(" ")[0]}</text>'''
