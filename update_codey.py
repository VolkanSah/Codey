...
def analyze_repo_quality(repo_data):
    ...
    # FIX: Plööt #4: Falsche Metrik korrigiert. 'has_downloads' ist kein Proxy für ein README.
    # Nutze 'has_wiki' als schwachen Proxy (oder entferne den Check, wenn kein direkter API-Aufruf für README gemacht wird)
    quality_factors = {
        'has_readme': bool(repo_data.get('has_wiki')), # Schwacher Proxy, da kein direkter 'has_readme' Feld existiert
    ...
    if not quality_factors['has_description']:
        score -= 0.2
    # FIX: Plööt #5: Brutale Fork-Abwertung gemildert von 0.1 auf 0.5 (immer noch brutal)
    if quality_factors['is_fork']:
        score *= 0.5  # Forks are worth less
...
def calculate_skill_decay(last_update_str, current_stats):
    ...
    try:
        ...
        days_inactive = (now_utc - last_update).days
        
        # FIX: Plööt #3: Zu Gnädiger Decay korrigiert. Decay beginnt, sobald es nicht am selben Tag ist.
        if (now_utc.date() - last_update.date()).days == 0:
            return current_stats # Kein Decay am selben Kalendertag
        
        # FIX: Plööt #3: Echter Verfall beginnt ab Tag 1
        # days_inactive muss mindestens 1 sein, wenn die Bedingung oben nicht zutrifft.
        days_of_decay = max(1, days_inactive) 
        
        # Exponential decay
        decay_factor = 0.95 ** days_of_decay
        
        decayed_stats = current_stats.copy()
        decayed_stats['health'] *= decay_factor
        decayed_stats['happiness'] *= decay_factor
        # FIX: Plööt #6: Decay-Bremse entfernt. Energie soll aggressiv verfallen.
        decayed_stats['energy'] *= decay_factor
...
def get_all_data_for_user(owner):
    ...
    # Language analysis (ALLE Repos für korrekte Statistik nutzen)
    languages_bytes = Counter()
    # FIX: Plööt #8: Statistik-Fälschung korrigiert. Sprachen-Analyse für ALLE Repos.
    ...
    for event in events[:50]:  # Analyze more events
        ...
        if not event_time_str:
            continue
        event_time = datetime.fromisoformat(event_time_str.replace('Z', '+00:00')) # Ensure timezone-aware comparison
...
def update_brutal_stats(codey, daily_activity, all_time_data, user_data):
    ...
    # FIX: Plööt #7: Logik-Aufspaltung korrigiert. Passive Degeneration zuerst.
    ...
    # Dann die positiven/negativen Effekte des täglichen Grinds
    codey['hunger'] = min(100, codey['hunger'] + commit_xp * 0.8 + pr_xp * 1.2)
    codey['happiness'] = min(100, codey['happiness'] + pr_xp * 0.6)
    # FIX: Plööt #1: Energie-Tyrann korrigiert. Passive Regeneration durch tägliche Aktivität entfernt.
    codey['energy'] = max(0, codey['energy'] - daily_activity['commits'] * 1.5 - daily_activity['prs'] * 3)
    
    # Brutale Health-Berechnung
...
def get_seasonal_bonus():
    ...
    # FIX: Plööt #2: November-Loch geschlossen.
    if month == 11: return {'emoji': '🦃', 'name': 'Thankful Grind', 'multiplier': 1.1}
...
def generate_brutal_svg(codey, seasonal_bonus):
    ...
    # FIX: Plööt #9: Zeit-Verwirrung korrigiert. Nutze den korrekten 'last_update' Wert und formatiere ihn.
    last_update_str = codey.get('last_update', 'Unknown')
    if last_update_str != 'Unknown':
        try:
            # ISO-Format parsen und neu formatieren
            last_update_dt = datetime.fromisoformat(last_update_str)
            last_update_display = last_update_dt.strftime('%Y-%m-%d %H:%M UTC')
        except ValueError:
    ...
    svg = f'''<svg width="630" height="473" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="315" y="438" text-anchor="middle" fill="{colors['secondary_text']}" font-family="Arial, sans-serif" font-size="12">
        Last Update: {last_update_display} • Dominant: {dominant_lang}
      </text>
