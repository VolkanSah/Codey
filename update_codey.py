...
def generate_brutal_svg(codey, seasonal_bonus):
    ...
    stats = {
        'h': (get_ascii_bar(codey.get('health', 0)), f"{codey.get('health', 0):.0f}%"),
        'm': (get_ascii_bar(codey.get('hunger', 0)), f"{codey.get('hunger', 0):.0f}%"),
        'ha': (get_ascii_bar(codey.get('happiness', 0)), f"{codey.get('happiness', 0):.0f}%"),
        'e': (get_ascii_bar(codey.get('energy', 0)), f"{codey.get('energy', 0):.0f}%"),
        's': (get_ascii_bar(brutal_stats.get('social_score', 1.0) * 50), f"{brutal_stats.get('social_score', 1.0):.2f}"),
        'q': (get_ascii_bar(brutal_stats.get('avg_repo_quality', 0.5) * 100), f"{brutal_stats.get('avg_repo_quality', 0.5):.2f}")
    }
    ...
    # 3. DAS ORIGINALE CUTY SVG (MIT DYNAMISCHEN INHALTEN)
    return f'''<svg width="630" height="473" xmlns="http://www.w3.org/2000/svg">
  <defs>
    ...
    <style>
      ...
      .led1 {{ animation: ledpop 0.9s infinite; }} .led2 {{ animation: ledpop 1.3s infinite 0.3s; }}
    </style>
  </defs>
  ...
  <text x="608" y="34" text-anchor="end" fill="#bf00ff" font-family="Courier New" font-size="10" opacity="0.5">{timestamp}</text>
  ...
  <g class="bot-body">
    ...
    <g class="head-bob">
      ...
      <circle cx="72" cy="140" r="14" fill="url(#headgrad)" filter="url(#glow)"/>
      <circle cx="152" cy="140" r="14" fill="url(#headgrad)" filter="url(#glow)"/>
  ...
  <g transform="translate(232, 50)" font-family="Courier New" fill="#e0aaff">
    ...
    <text x="8" y="35" font-size="11" font-weight="bold">[{tier.upper()}] LVL {codey['level']} • {stars_str} PRESTIGE {prestige_lv}</text>
    
    <g transform="translate(0,64)" font-size="12">
      <text x="0" y="0" opacity="0.65">health</text><text x="80" y="0">[{stats['h'][0]}]</text><text x="374" y="0" text-anchor="end" fill="#ff88dd">{stats['h'][1]}</text>
      <text x="0" y="22" opacity="0.65">hunger</text><text x="80" y="22">[{stats['m'][0]}]</text><text x="374" y="22" text-anchor="end" fill="#ff88dd">{stats['m'][1]}</text>
      <text x="0" y="44" opacity="0.65">happiness</text><text x="80" y="44">[{stats['ha'][0]}]</text><text x="374" y="44" text-anchor="end" fill="#ff88dd">{stats['ha'][1]}</text>
      <text x="0" y="66" opacity="0.65">energy</text><text x="80" y="66">[{stats['e'][0]}]</text><text x="374" y="66" text-anchor="end" fill="#ff88dd">{stats['e'][1]}</text>
      <text x="0" y="88" opacity="0.65">social</text><text x="80" y="88">[{stats['s'][0]}]</text><text x="374" y="88" text-anchor="end" fill="#ff88dd">{stats['s'][1]}</text>
      <text x="0" y="110" opacity="0.65">quality</text><text x="80" y="110">[{stats['q'][0]}]</text><text x="374" y="110" text-anchor="end" fill="#ff88dd">{stats['q'][1]}</text>
    </g>
