...
def generate_brutal_svg(codey, seasonal_bonus, cycles=4):
    ...
    # ── STATUS: quality_curator in social_penalties = Bonus, rest = Penalty ──
    social_penalties = brutal_stats.get('social_penalties', [])
    BONUS_TAGS = {'quality_curator'}
    actual_penalties = [p for p in social_penalties if p not in BONUS_TAGS]
    actual_bonuses   = [p for p in social_penalties if p in BONUS_TAGS]

    if actual_bonuses:
        status_val   = actual_bonuses[0]
        status_color = '#22cc66'
    elif actual_penalties:
        status_val   = actual_penalties[0]
        status_color = '#ff4444'
    else:
        status_val   = 'clean'
        status_color = '#8b949e'

    # ── INFLATION: star_per_repo aus verfügbaren Werten ───────────────────
    total_own_repos = sum(
        1 for r in [] if not r.get('fork')
    )  # nicht in brutal_stats — Näherung:
    # total_stars / max(level,1) als proxy, oder einfach weglassen
    # Wir zeigen: STARS/REPO ratio wenn sinnvoll
    star_per_repo = brutal_stats.get('total_stars', 0) / max(codey.get('level', 1), 1)

    season_info = (
        f'SEASON={seasonal_bonus["emoji"]} {seasonal_bonus["name"]} +{int((seasonal_bonus["multiplier"]-1)*100)}%'
        if seasonal_bonus else 'SEASON=OFFLINE'
    )

    # ── Issues — nicht im Core, komplett raus ─────────────────────────────
    issue_xml = ''
  ...
  <g transform="matrix(1.199,0,0,1.145,267,274)" font-family="Courier New,monospace" fill="#e0aaff" font-size="12">
    ...
    <text x="0" y="42">STARS={total_stars}  •  SPR={star_per_repo:.1f}  •  {dominant_lang} {pet_emoji}</text>
